...
class AnalysisAgent:
    ...
    def _init_state(self):
        ...
        if 'tiers_used' not in st.session_state:
            st.session_state.tiers_used = {}
    ...
    def analyze_report(self, data, system_prompt, check_only=False, chat_history=None, preferred_tier=None):
        """
        ...
        Args:
            ...
            preferred_tier: Optional ModelTier (or tier value string) to start the
                model cascade from, based on the user's speed/quality preference.
        """
        ...
        # Resolve a string preference (e.g. "secondary") into a ModelTier
        start_tier = preferred_tier
        if isinstance(start_tier, str):
            try:
                start_tier = ModelTier(start_tier)
            except ValueError:
                start_tier = None
        
        # Generate analysis using model manager
        result = self.model_manager.generate_analysis(
            processed_data, enhanced_prompt, start_tier=start_tier
        )
    ...
    def _update_analytics(self, result):
        ...
        # Track which cascade tier actually served the request
        tier_used = result.get("tier_used", "unknown")
        st.session_state.tiers_used[tier_used] = st.session_state.tiers_used.get(tier_used, 0) + 1
